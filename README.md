...
- **Dynamic Mapping**: Leaflet integration showing color-coded monitoring stations, with a "Live" pulse indicator in the header that reads "Delayed" when the newest data is more than 3 hours old.
- **Historical Analysis & Trends**: 
    - **Daily Lookback**: Select any past date to review AirNow's daily summaries (preliminary data, not for NAAQS compliance).
    - **10-Day Trends**: Interactive Chart.js visualizations for individual pollutants (Ozone, PM2.5, PM10, CO, SO2).
...
- **Secure Server-Side Proxy**:
    - **Credential Injection**: Protects sensitive AirNow API keys by injecting them server-side, ensuring they are never exposed to the client. The key is read from `AIRNOW_API_KEY` (for `npm run dev`, put it in `.env.local`); only the Forecast tab needs it.
...
- **Lazy Loading**: The heavy map components are dynamically imported to minimize initial page load.
...
- **Mapping**: [React-Leaflet](https://react-leaflet.js.org/) on Esri's keyless World Light/Dark Gray Canvas basemaps (Powered by Esri)
